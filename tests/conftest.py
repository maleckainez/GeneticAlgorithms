...
def experiment_config_factory() -> Callable[..., ExperimentConfig]:
    """Create a fully initialized ExperimentConfig fixture for tests.
...
class DummyPopManager:
    """Lightweight stand-in for PopulationHandler used in fitness adapter tests.

    This helper stores an in-memory population array and exposes only the
    ``get_pop_handle`` method expected by ``fitness_class_adapter``. It avoids
    file-backed memmaps while keeping the interface compatible with the real
    population manager.
    """
    ...
    def get_pop_handle(self) -> np.ndarray:
        return self._population


@pytest.fixture
def dummy_pop_manager() -> Callable[[np.ndarray], DummyPopManager]:
    """Fixture returning a factory producing DummyPopManager instances for tests.

    Returns:
        Callable[[np.ndarray], DummyPopManager]: Factory that wraps a provided
        population array inside a DummyPopManager so tests can supply a mock
        population handler without disk I/O.

    """

    def _factory(population: np.ndarray) -> DummyPopManager:
        return DummyPopManager(population)
